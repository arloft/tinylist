...
theAnswer = input("Do you want to add anything to it (y/n)?")
# directs the program based on whether you want to add stuff, or leave the file as it is, without changes

if theAnswer == "y":
    clearTheFile = input("Do you want to clear the file contents (y/n)?")
    if clearTheFile == "y":
        theFile.truncate(0)
    ...
    addThisToTheFile = input("> ")
    if clearTheFile == "y":
        theFile.write(addThisToTheFile)
    else:
        theFile.write("\n"+addThisToTheFile)
        # add more than one line to the file
        continueWriting = input("Add another line? (y/n)?")
